...
model_urls = {
    "google_drive": "https://drive.google.com/drive/folders/1azgrD1P413pXLJME0PjRRU-Ez-4GWN-S"
}
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        ...
        # 是一种编码技巧，比如我们要加深网络，有些层是不改变输入数据的维度的，在增减网络的过程中我们就可以用identity占个位置，这样网络整体层数不变
...
class Attention(nn.Module):
    def __init__(self,
                 dim,
                 num_heads=8,
                 qkv_bias=False, # qkv_bias表示qkv是否使用偏置参数
                 qk_scale=None,
                 attn_drop_ratio=0.,
                 proj_drop_ratio=0.):
    ...
    def forward(self, x):
        ...
        # permute函数可以对任意高维矩阵进行转置，调整维度顺序
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # [batch_size, num_heads, num_patches + 1, embed_dim_per_head]
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale # @表示矩阵乘法,transpose(-2, -1)将最后两个维度进行调换
        attn = attn.softmax(dim=-1) # dim=-1表示针对矩阵每一行进行softmax处理
        attn = self.attn_drop(attn) # dropout层

        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # 得到的结果和V矩阵点乘（加权求和），reshape相当于把head拼接
        x = self.proj(x)  # 通过全连接进行映射（相当于乘论文中的Wo）
        x = self.proj_drop(x) # dropout层
        return x
...
class Block(nn.Module):
    def __init__(self,
                 dim,
                 num_heads,
                 mlp_ratio=4., # MLP中hidden_features是in_features的四倍
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)  # MLP第一个全连接层的隐层节点个数
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop_ratio)

    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x))) # 残差结构
        x = x + self.drop_path(self.mlp(self.norm2(x))) # 残差结构
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        self.num_tokens = 2 if distilled else 1 # 用于分类的[class]token，一般是1
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        act_layer = act_layer or nn.GELU # 如果没有传入act_layer就默认nn.GELU
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim)) # 位置embedding
...
def vit_base_patch16_224(num_classes: int = 1000):
    ...
    ImageNet-1k SGD0.0005_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    """
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k SGD0.0005_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model = VisionTransformer(img_size=224,
                              patch_size=16,
                              embed_dim=768,
...
def vit_large_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k SGD0.0005_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    SGD0.0005_weights ported from official Google JAX impl:
...
def vit_huge_patch14_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    ImageNet-21k SGD0.0005_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: converted SGD0.0005_weights not currently available, too large for github release hosting.
    """
    model = VisionTransformer(img_size=224,
                              patch_size=14,
                              embed_dim=1280,
