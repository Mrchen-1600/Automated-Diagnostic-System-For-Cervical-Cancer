...
from VisionTransformer_Model import vit_base_patch16_224_in21k
...
def load_model(device, has_logits, num_classes, model_path):
    """加载模型及权重"""
    model = vit_base_patch16_224_in21k(num_classes=num_classes, has_logits=has_logits).to(device)
...
def main():
    ...
    model = load_model(device, opt.HASLOGITS, opt.NUM_CLASSES, opt.BEST_MODEL_PATH)
